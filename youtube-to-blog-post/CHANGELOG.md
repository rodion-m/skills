...
- 对齐短裤发布流水线 v2.40.2，禁止重复且缺字段的 Microdata 视频实体进入发布结果。

# v4.8.1 - 2026-07-28

- 对齐博客站点 v2.3.1：独立观看页的播放器必须位于 H1 和说明文字之前。
- 对齐短裤发布流水线 v2.40.1 的播放器突出度门禁，避免长标题把视频推离首屏主体位置。

# v4.8.0 - 2026-07-28

- 对齐博客站点 v2.3.0：Markdown 中的 iframe 继续作为观看页生成源，最终文章 HTML 只展示可点击的视频封面入口。
- 文章页保留 BlogPosting、Twitter/X 大图和完整正文，但不再输出播放器或 VideoObject。
- 真正的 YouTube 播放器、VideoObject、视频 Sitemap 身份与文章回链仅由 `/videos/<video_id>/` 独立观看页承载。
- 对齐短裤发布流水线 v2.40.0 的双向页面职责门禁，避免后续生成内容重新触发“视频不在观看页面上”。

# v4.7.1 - 2026-07-27

- 明确生成文章的 Hexo 永久链接会自动进入 `/videos/<video_id>/` 播放器下方的“对应博客文章”区域。
- 该入口明确承载完整步骤、命令、资源链接和补充说明，不需要生成器重复维护另一份链接。
- 发布门禁验证链接为站内非视频 URL，并要求对应文章 HTML 已生成且非空。
