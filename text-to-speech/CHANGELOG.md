...
- 校验每段文本、字符范围、时间范围和全局单调性；异常或缺失时阻断合成结果。
- 本地 sidecar 不保存短期签名下载 URL，只保存下游音画同步所需的经过校验字段。
- Edge/Kokoro 显式拒绝时间戳 sidecar 参数，禁止用非官方估算数据冒充 MiniMax 时间戳。
- 增加 sidecar 下载、清洗、时间异常和引擎隔离测试。
